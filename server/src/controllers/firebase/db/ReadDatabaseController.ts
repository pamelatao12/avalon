...
export const AddEventListenerController = (eventType: EventType, path: string): void => {
  const currRef = db.ref(path)
  currRef.on(eventType, (snapshot: any | null, prevChildKey?: string | undefined) => {
...
export const GetStateController = (path: string = 'server/poker'): object => {
  ...
  return currRef.once("value", (snapshot) => {
    console.log('this is what the data looks like now: ', snapshot.numChildren())
